...
export default function AddEditOfferModal({ isOpen, onClose, onSubmit, category }) {
  ...
  useEffect(() => {
    if (category) {
      const hasOffers = category.offers && category.offers.length > 0;
      const selectedOffer = hasOffers ? category.offers[0] : null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (category?.offers?.[0]?._id) {
        formattedData._id = category.offers[0]._id;
      }
  ...
  const isEditing = Boolean(category?.offers?.[0]?._id);
