...
const ProductCard = ({
  ...
  const checkWishlistStatus = async () => {
    try {
      ...
      const isItemInWishlist = wishlistItems.some((item) => item._id === id || item.product?._id === id)
  ...
  const handleWishlistClick = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.message || "Failed to update wishlist. Please try again.")
  ...
  const imageUrl = images && images.length > 0 ? images[0] : "/placeholder.svg?height=300&width=300"
  ...
  return (
    <div className="group relative bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 p-2">
      ...
      <div className="relative aspect-square">
        ...
        >
          <Heart className={`h-4 w-4 sm:h-5 sm:w-5 ${localIsInWishlist ? "fill-current" : ""}`} />
          <span className="sr-only">{localIsInWishlist ? "Remove from wishlist" : "Add to wishlist"}</span>
        </button>
      ...
      <div className="p-2 sm:p-4">
        <div onClick={handleProductClick} className="block cursor-pointer">
          <h3 className="text-sm sm:text-lg font-semibold text-gray-900 mb-1 hover:text-blue-600 transition-colors duration-300">
            {name || "Product Name"}
          </h3>
          ...
          <div className="flex items-center mb-2">
            ...
            <span className="ml-2 text-xs sm:text-sm text-gray-600">({reviewCount} reviews)</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              {discountPercent > 0 && price != null && (
                <span className="ml-2 text-xs sm:text-sm text-gray-500 line-through">₹{price.toFixed(2)}</span>
              )}
            </div>
            {discountPercent > 0 && (
      ...
      <div className="p-2 sm:p-4 pt-0">
        <button
          ...
          className={`block w-full text-center rounded-md py-2 px-3 text-xs sm:text-sm font-medium 
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal}>
        <h2 className="text-xl sm:text-2xl font-bold mb-4">Add to Wishlist</h2>
        <p className="mb-6 text-sm sm:text-base">Please log in to add items to your wishlist.</p>
        <div className="flex justify-between">
          <button
            onClick={navigateToLogin}
            className="bg-blue-500 text-white px-3 py-1 sm:px-4 sm:py-2 rounded text-sm sm:text-base hover:bg-blue-600 transition-colors"
          >
            Log In
          </button>
          <button
            onClick={closeModal}
            className="bg-gray-300 text-gray-700 px-3 py-1 sm:px-4 sm:py-2 rounded text-sm sm:text-base hover:bg-gray-400 transition-colors"
          >
            Cancel
          </button>
        </div>
