...
const CheckoutPage = () => {
  ...
  const calculateOrderSummary = (items) => {
    ...
    if (appliedCoupon) {
      if (appliedCoupon.discountType === "percentage") {
        discount = subtotal * (Number.parseFloat(appliedCoupon.discountValue) / 100)
      } else if (appliedCoupon.discountType === "fixed") {
        discount = Number.parseFloat(appliedCoupon.discountValue)
      }
    }
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        const subtotal = Number.parseFloat(orderSummary.subtotal)
        const tax = Number.parseFloat(orderSummary.tax)
        const shipping = Number.parseFloat(orderSummary.shipping)
        const discountAmount = Number.parseFloat(response.discountAmount)
  ...
  }, [cartItems, appliedCoupon, checkCouponValidity]) // Added checkCouponValidity to dependencies
  ...
  if (addresses.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-100 to-indigo-100 flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-md w-full">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">No Delivery Addresses Found</h2>
            <p className="text-gray-600 mb-6">Please add a delivery address to continue checkout.</p>
            <button
                    ...
                    {paymentMethods.map((method) => (
                      ...
                      >
                        ...
                        {method.id === "cash_on_delivery" && (
                          <p className="text-xs text-gray-600 text-center mt-1">Pay when your order arrives</p>
                        )}
                      </div>
                    ))}
                  </div>
                    ...
                    {appliedCoupon && (
                      <div className="mt-4 bg-green-50 border border-green-200 rounded-xl p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-semibold text-green-800">Applied: {appliedCoupon.code}</p>
                            <p className="text-sm text-green-600">{appliedCoupon.discountValue}% off</p>
                          </div>
                          <button
                            type="button"
                            onClick={handleRemoveCoupon}
                            className="text-red-600 hover:text-red-800 font-medium transition duration-300 px-3 py-1 rounded-lg hover:bg-red-50"
                          >
                            Remove
                          </button>
                        </div>
              ...
              <div className="bg-gray-50 rounded-2xl p-6 h-fit sticky top-8">
                ...
                <div className="mt-8">
                  <button
                    ...
                    className={`w-full py-4 rounded-xl text-white font-semibold text-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 ${
