  ...
  return (
    <div className="container mx-auto px-4">
      ...
      <ConfirmationModal
        ...
        title={`Confirm ${modalAction.type === 'edit' ? 'Edit' : modalAction.type === 'toggleStatus' ? (products.find(p => p._id === modalAction.id)?.isactive ? 'Block' : 'Unblock') : ''}`}
        message={`Are you sure you want to ${modalAction.type === 'edit' ? 'edit' : modalAction.type === 'toggleStatus' ? (products.find(p => p._id === modalAction.id)?.isactive ? 'block' : 'unblock') : ''} this product?`}
      />
    </div>
  )
}
